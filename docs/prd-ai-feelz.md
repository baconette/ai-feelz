...
**Status:** Draft v0.2 — Early Exploration
...
1. **Not a research or survey platform** — this is a consumer experience product, not an academic data collection tool. Rigorous sampling, demographic weighting, and longitudinal tracking are out of scope for v1.
...
- Upon completing ratings for a domain (or all domains), the visitor sees a personalized visualization of their results.
...
**7. Demographic slicing (self-reported)**
- Visitors can optionally self-identify (age range, relationship to AI in their work) and see aggregate data filtered by peer group.
...
- Registered users can take the experience multiple times and see how their AI attitudes shift over time, with a "your attitudes over time" visualization.
...
|---|---|---|---|
| 1 | **Rating scale / narrative framing**: Which axis (or axes) will visitors rate on? Options: (a) human ↔ AI, (b) uneasy ↔ enthusiastic, (c) should exist ↔ should not exist, (d) replaces ↔ augments, (e) trust ↔ distrust. Shapes data model and all visualizations. | **Design** | 🔴 Action item — blocks data model |
| 2 | **Individual results viz type**: What visual form best represents an individual's AI attitude profile? To be decided after #1 and domain list are confirmed. | **Design** | 🔴 Action item — blocks front-end build |
| 3 | **Use case list per domain**: What are the specific use cases within each domain? Should any be intentionally provocative to create differentiation in responses? | **Design + Content** | 🔴 Action item — blocks rating module per epic |
| 4 | **Aggregate minimum threshold**: How many responses before aggregate data is shown? What placeholder state is shown before the threshold is met? | **Design + Engineering** | 🔴 Action item — blocks aggregate view |
| 5 | **Anchoring bias mitigation**: Aggregate shown after individual results only, or offer a toggle? What if the visitor wants to see aggregate *while* rating? | **Design** | 🟡 Non-blocking — affects UX flow |
| 6 | **Shareability format**: Unique URL only (v1), or also a downloadable static image? Static image requires server-side rendering infrastructure and is a scope decision. | **Engineering** | 🟡 Non-blocking — URL confirmed for v1, image is P1 |
| 7 | **Mobile responsiveness**: Desktop-first with graceful mobile degradation, or full mobile parity from the start? | **Design + Engineering** | 🟡 Non-blocking — affects build scope |
| 8 | **"Attitude profile" framing**: Labeled archetype vs. purely visual/quantitative profile? Archetypes risk oversimplification. | **Design + Content** | 🟢 P1 — decide before Phase 2 |
| 9 | **Domain data architecture**: Domains and use cases must be stored as data in Supabase (not hardcoded in the app) so that adding a new domain is a database write, not a code change. Prerequisite for the P0 requirement that new domains can be added without re-deploying. | **Engineering** | 🔴 Action item — blocks domain increment architecture |
...
| **Bill the Patriarchy** | [billthepatriarchy.com](https://www.billthepatriarchy.com/) | TBD |

**Design sprint prompt:** Review these with specific attention to (1) how they handle the moment between input and results — the reveal — and (2) how they make aggregate or benchmark data feel relevant to the individual rather than generic.
...
- Recommended sequencing:
  ...
  - **Epic 1:** Rating module + individual results viz + aggregate + share link for Domain 1.
